...
\title{Constrained AI-Assisted Sampling for Fragmented Textual Spaces: \\ A Framework for Data Collection Where No Ground Truth Exists}
\subtitle{DRAFT PRE REVIEW}
\addauthor{Lalitha A R}{Lead Researcher, Interdisciplinary Systems Research Lab (iSRL)}
\docid{iSRL-26-03-R-CAAS}
\license{Licensed under CC BY 4.0}
\date{March 2026}
\coverabstract{%
Structured data collection typically assumes either a population that can be enumerated or a schema that can be targeted. Many domains satisfy neither condition: the information exists, scattered across unstructured text, but no authoritative reference organises it and no standard cleaning pipeline can handle the domain-specific variation reliably. This report describes Constrained AI-Assisted Sampling (CAAS), a framework for data collection in these conditions. The core argument is not methodological novelty but an asymmetry in error economics: when source data already exists in retrievable form, the cost of correcting a wrong extraction is a refetch, not a repeated physical act. This asymmetry makes constrained AI retrieval viable even without determinism guarantees, provided failure is explicit rather than silent and validation is statistical rather than exhaustive. Two independent implementations of this approach --- one building an ingredient vocabulary for Indian packaged food labels, the other constructing a historical record of global flash floods from news archives --- arrived at the same architectural decisions without coordination. That convergence is the primary evidence this report presents.%
}
...
A fragmented textual space is not simply messy data. It is a domain where the information exists --- recorded somewhere, in some form --- but is distributed across unstructured sources with no shared vocabulary, no authoritative lexicon, and variation patterns that automated similarity measures cannot reliably navigate. The Indian packaged food label space is one example: the same ingredient appears as \texttt{maida}, \texttt{refined wheat flour}, and \texttt{all-purpose flour} across different brands, while \texttt{palm oil} and \texttt{palmite} look similar but are functionally distinct. A global news archive is another: 2.6 million flood events are embedded in articles across 80 languages, with relative time references, imprecise location language, and no standardised event schema.
...
% ---------------------------------------------------------------
\section{Why AI as a Constrained Parser, Not a Generator}
% ---------------------------------------------------------------
...
% ---------------------------------------------------------------
\section{The Cost of Error Correction}
% ---------------------------------------------------------------
...
\subsection{One Atomic Operation Per API Call}
...
\subsection{Explicit Failure Over Approximation}
...
In the ingredient extraction implementation, the system instruction read: \textit{``If ingredient list not found in whitelisted domains, return DATA\_NOT\_FOUND. DO NOT infer typical ingredients from product category. DO NOT approximate based on similar products.''}

Of 1,000 products attempted, 104 returned persistent \texttt{DATA\_NOT\_FOUND} across two passes. These 104 were excluded from the corpus. The exclusion is a feature: those products either had no verifiable online ingredient list or were no longer in active distribution. The pipeline returned a clean gap rather than 104 fabricated entries that would have required expensive downstream correction.
...
\subsection{Batch Size as a Quality Variable}
...
\subsection{Iterative Human-in-the-Loop Audit}
...
\subsection{Statistical Validation with Finite Population Correction}
...
% ---------------------------------------------------------------
\section{Two Domains, Same Architecture}
% ---------------------------------------------------------------
...
\toprule
\textbf{Decision} & \textbf{Ingredient vocabulary} & \textbf{Flood record} \\
\midrule
Model role & Retrieval and parsing only & Classification, temporal anchoring, spatial extraction \\[4pt]
Source constraint & Whitelisted domains in priority order & News reports where flooding is primary subject \\[4pt]
Failure handling & \texttt{DATA\_NOT\_FOUND} token & Explicit classification criteria; non-flood articles return nothing \\[4pt]
Batch calibration & 6 SKUs per batch (empirical) & Per-article processing with complexity-aware handling \\[4pt]
Validation & Statistical sampling + iterative audit & Manual review sample; spatiotemporal matching against reference archive \\[4pt]
Accuracy result & 98.9\% at 95\% confidence & 82\% practically useful; 85--100\% severe event recall \\
\bottomrule
\end{tabular}
\caption{Architectural decisions across two independent CAAS implementations.}
\end{table}
...
% ---------------------------------------------------------------
\section{What This Does Not Guarantee}
% ---------------------------------------------------------------
...
The framework does not remove the need for domain judgment. In the ingredient implementation, boundary cases --- whether \texttt{gluten} belongs in grains or proteins, whether \texttt{spirulina} is an additive or a botanical --- were not resolvable through cleaning. They required a classification framework with explicit criteria for how those categories are defined. CAAS reduces the volume of decisions that require human judgment. It does not eliminate the decisions themselves.
...
\bibitem{groundsource2026}
R.~Mayo, O.~Zlydenko, D.~Cohen, \textit{et al.}
\newblock Groundsource: A dataset of flood events from news.
\newblock EarthArXiv, March 2026.
\newblock \url{https://doi.org/10.31223/X5RR2K}.


\bibitem{ifidSamplingCorpus2026}
L.~A.~R.
\newblock [IFID Sampling Corpus --- placeholder, fill with Zenodo DOI].
\newblock Interdisciplinary Systems Research Lab (iSRL), 2026.

\bibitem{cochran1977}
W.~G.~Cochran.
\newblock {\em Sampling Techniques}.
\newblock John Wiley \& Sons, 3rd edition, 1977.

\bibitem{julious2019}
S.~A.~Julious.
\newblock Calculation of confidence intervals for a finite population size.
\newblock {\em Pharmaceutical Statistics}, 18(1):115--121, 2019.
